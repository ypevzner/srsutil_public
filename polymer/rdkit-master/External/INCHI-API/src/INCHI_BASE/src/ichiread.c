...
char * ParseSegmentReadDelimitedNumbers( const char *str, const char *pEnd,
                   ...
                   OAD_V3000 **ppV3000 )
{
    ...
    bAbc = 0;
    /* We do not support compressed InChI explicitly since at least */
                      ...
                      int        *pbAbc )
{
    ...
    char *p, *q, *pStart, *pEnd, *pPermStart, *pPermEnd;
    int  ret = 0;
...
int ParseSegmentPolymer( const char  *str,
                         ...
                         OAD_V3000   **ppV3000 )
{
    char  *p = NULL, *q = NULL, *pStart = NULL, *pEnd = NULL;
    char  comma = ',', dot = '.', dash = '-', lt_par = '(', rt_par = ')', *p0;
...
exit_function:
    ...
    else
    {
        if (pd)
        {
            ...
            for (iu = 0; iu < pd->n; iu++)
            {
                int astar1 = pd->units[iu]->cap1, astar2 = pd->units[iu]->cap2;
                if (0 == pd->units[iu]->nb)
                {
                    continue;
                }
...
****************************************************************************/
char* ParseSegmentReadDelimitedNumbers( const char  *str,
                                        const char  *pEnd,
                                        INT_ARRAY   *numlist,
                                        char        c_delim,
                                        char        c_stop,
...
#define nNum_H( ICOMPONENT ) ((bMobileH==TAUT_YES)? pInChI[ICOMPONENT].nNum_H : pInChI[ICOMPONENT].nNum_H_fixed)
    ...
    char *p, *q, *h, *t, *p1, *pTaut, *pStart, *pEnd;
