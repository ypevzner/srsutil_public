                                     ...
                                     char *pTAB )
{
    ...
    int nat = orig_inp_data->num_inp_atoms;
    int num_inp_bonds = orig_inp_data->num_inp_bonds;
...
void EditINCHI_HidePolymerZz(INCHI_IOSTREAM *out, int n_pzz, int n_zy)
{
    ...
    for (i = start; i < out->s.nUsedLength; i++)
    {
        ...
        if (is_in_z_layer && !skip)
        {
            if (s[i] == '(')
            {
                /* 1.06 : skip pattern "(cap,cap-bkbonds)" but not "(cap-end, cap-end)" */
                char *q, *p = out->s.pStr + i + 1;
