...
#error Wrong initialization
    ...
    *ulDisplTime = 0;
    ...
    ip->bAllowZz = bAllowZz;
    if (!ip->bAllowZz && ip->bPolymers)
    {
        ip->bAllowZz = 0;
    }
    ip->bStereoAtZz = bStereoAtZz;
                     ...
                     INPUT_PARMS *ip )
{
    ...
    if (ip->bPolymers)
    {
        ...
        if (ip->bExpoZz == EXPO_ZZ_MINOR)
            inchi_ios_eprint( log_file, "; expose Zz atoms if present (minor appearance)" );
        else if (ip->bExpoZz == EXPO_ZZ_TOTAL)
            inchi_ios_eprint( log_file, "; expose Zz atoms if present (total appearance)" );
        inchi_ios_eprint( log_file, "\n" );
    }
    if (ip->bAllowZz)
    {
        inchi_ios_eprint( log_file, "Experimental mode: Zz (star) atoms allowed" );
        if (ip->bStereoAtZz)
        {
