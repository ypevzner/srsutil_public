...
namespace FDA.SRS.Processing
{
    ...
    {
        public static List<Tuple<OutputFileType, string>> ProtPlmr2Spl(ImportOptions impOpt, ConvertOptions opt, ExportOptions expOpt)
        {
            ...
            foreach (string file in pars.InputFiles)
                ProtPlmr2Spl(file, impOpt, opt, expOpt, pars, result);

            return result;
        }

        private static void ProtPlmr2Spl(string file, ImportOptions impOpt, ConvertOptions opt, ExportOptions expOpt, OperationalParameters pars, List<Tuple<OutputFileType, string>> result)
        {
                                    ...
                                    foreach (Chain chn in plmr.Subunits)
                                    {
                                        foreach (SRU chn_sru in chn.SRUs)
                                        {
                                            if (chn_sru.Amount.SrsAmountType == "MOLE PERCENT")
                                            {
                                                ...
                                                {
                                                    chn_sru.Amount.Center = Math.Round((double)chn_sru.Amount.Center * (double)plmr.polimerization_factor, 2);
                                                }
                                                
                                                if (chn_sru.Amount.Low != null)
                                                {
                                                    chn_sru.Amount.Low = Math.Round((double)chn_sru.Amount.Low * (double)plmr.polimerization_factor, 2);
                                                }
                                                if (chn_sru.Amount.High != null)
                                                {
                                                    chn_sru.Amount.High = Math.Round((double)chn_sru.Amount.High * (double)plmr.polimerization_factor, 2);
                                                }
                                            }
                                        }
                                    }
