...
{
	public class NASequence : SplObject, IUniquelyIdentifiable
	{
		private string _id;
		...
		public string UID
		{
			...
			{
				if ( _uid == null )
					_uid = String.Join("|", _sequence.Select(l => Nucleotides.GetCompoundByLetter(l)).Select(c => c.UID)).GetMD5String();
				return _uid;
			}
		}
		...
		public NASequence(SplObject rootObject, string sequence, string id = null, string name = null)
			: base(rootObject, id)
		{
			...
			if ( sequence.Any(c => String.IsNullOrEmpty(Nucleotides.GetNameByLetter(c))) )
...
//end YB
		...
		public XElement SequenceSpl
		{
			get
			{
				return new SplCharacteristic("NUCLEIC ACID SEQUENCE", _sequenceOriginal).SPL;
			}
		}
